...
ADD = f"INSERT INTO {TABLE_NAME} (post_text, parent_id, user_id, creator_id) VALUES (%s, %s, %s, %s) RETURNING *;"
GET = f"SELECT * FROM {TABLE_NAME} WHERE id=%s;"
GET_BY_USER = f"SELECT * FROM {TABLE_NAME} WHERE user_id=%s;"
UPDATE = f"UPDATE {TABLE_NAME} SET post_text = %s, post_image = %s, like_count = %s, timestamp_ms_updated = %s WHERE id=%s"
